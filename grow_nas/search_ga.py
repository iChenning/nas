...
# 是否满足约束条件
def is_satisfy_consts(consts, choose, all_choose, rate=(0.95, 1.05)):
    ...
    if choose not in all_choose:
        ...
        if consts[0] * rate[0] < arch_macs < consts[0] * rate[1] \
            ...
            # print('cur:', arch_macs, arch_params)
            return True
        else:
            return False
    else:
...
# 种子
def init_popu(consts_choose, stages, channel_scales, kernels, seed_len=40, rate=(0.95, 1.05)):
